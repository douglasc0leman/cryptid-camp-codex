        ...
        <div className={`p-6 flex items-center justify-center border-r border-gray-200 ${isLandscape ? 'md:w-[48%]' : 'md:w-1/3'}`}>
          <button onClick={() => setIsModalOpen(true)} className="focus:outline-none">
            <div className="relative group cursor-zoom-in transition-transform duration-200 transform hover:scale-105">
              ...
              <div className={`absolute inset-0 flex items-center justify-center ${isLandscape ? 'w-[360px] h-[270px]' : 'w-[364px] h-[504px]'} bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity duration-200`}>
          ...
          <div className="space-y-4">
            <div className="flex items-start justify-between gap-4">
              {card.cost !== null && (
                <div className="relative group w-14 h-14 shrink-0">
                  <div className={`w-full h-full rounded-full flex items-center justify-center font-extrabold text-2xl leading-none shadow border transition-transform duration-200 group-hover:scale-110 ${needsDarkText ? 'bg-gray-200 text-gray-800 border-gray-300' : 'bg-white/20 text-white border-white/30'}`}>{card.cost}</div>
                  <div className="absolute top-full mt-2 left-1/2 -translate-x-1/2 w-max px-3 py-1 rounded bg-black text-white text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-50 shadow-md whitespace-nowrap">Requires {card.cost} Vision to play</div>
                </div>
              )}
            ...
            {(card.is_common || card.is_uncommon || card.is_rare || card.is_unique) && (
              <div className="ml-[72px] mt-2 text-sm">
                ...
                {card.is_unique ? 'You can have 1 copy per deck.' : card.is_rare ? 'You can have up to 2 copies per deck.' : card.is_uncommon ? 'You can have up to 3 copies per deck.' : 'You can have up to 4 copies per deck.'}
              </div>
            )}
              ...
              <div className="flex flex-col pl-[60px] items-center sm:items-start">
                {(card.attack !== null || card.defense !== null) && (
                  <div className="inline-flex overflow-hidden rounded-full shadow text-white text-lg font-semibold w-fit">
                    <div className="bg-red-200 text-red-900 px-4 py-2">{card.attack ?? 0} ATK</div>
                    <div className="bg-blue-200 text-blue-900 px-4 py-2 border-l border-white/40">{card.defense ?? 0} DEF</div>
                  </div>
                )}
                {card.advantage && <div className="text-sm font-medium italic mt-1 text-center w-full">{card.advantage}</div>}
              </div>

              {taxons.length > 0 && (
                <div className="flex flex-wrap gap-3 pt-[2px] sm:pt-[2px]">
                  {taxons.map((t) => (
                    <span key={t} className={`px-4 py-1.5 rounded-full text-sm font-semibold tracking-wide ${needsDarkText ? 'bg-gray-200 text-gray-800' : 'bg-white/20 text-white border border-white/20'}`}>{t}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="px-18">
              ...
              {(card.illustrator || card.set_name || card.set_number) && (
                <div className="mt-4 flex justify-between text-base font-medium">
                  <span>{card.illustrator ? `Illustrator: ${card.illustrator}` : ''}</span>
                  <span>{card.set_name ? `Set: ${card.set_name}` : ''}</span>
                  <span>{card.set_number ? `Card #: ${card.set_number}` : ''}</span>
                </div>
              )}
            </div>
