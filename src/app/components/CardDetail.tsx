...
export default function CardDetail({ card }: { card: CryptidCampCard }) {
  ...
  // ✅ Smart detection for light backgrounds
  const cabin = card.cabin?.toLowerCase() || ''
  const needsDarkText =
  ['#eaf4ff', '#edf2f7', '#ffffff'].includes(bg.toLowerCase()) ||
  ['meteorite', 'corallium', 'fluorite', 'fulgurite', 'quartz'].includes(cabin)
    ...
    <div className="min-h-screen relative bg-gray-100 p-6 md:p-12 overflow-hidden">
      ...
      >
        ...
        <div className="md:w-2/3 p-6 space-y-4 relative">
          ...
          {card.flavor_text && (
            <div className={`italic mt-4 border-l-4 pl-4 ${needsDarkText ? 'text-black/70 border-black/20' : 'text-white/70 border-white/40'}`}>
              “{card.flavor_text}”
            </div>
          )}
          ...
          <div className="pt-6">
            <Link
              href={`/?page=${page}`}
              className={`${needsDarkText ? 'text-black' : 'text-white'} hover:underline font-medium text-sm`}
            >
